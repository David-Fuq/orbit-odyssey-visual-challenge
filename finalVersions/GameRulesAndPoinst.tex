...
The rules below are exhaustive. Any situation not explicitly permitted is \textbf{prohibited}. If an ambiguity arises during competition, the head referee's ruling is final and binding.
...
Field Performance is the sum of all object engagement points. This part of the score is given by the judges during the competition. It quantifies the performance of the robot during the challenge itself. Objects may be recognized and scored \textbf{more than once} --- there is no cap on the number of times a single object can be engaged.
...
\subsection*{E) Performance Bonus (max $+30$)}

Performance Bonuses are awarded at the end of the round based on the overall performance. They are \textbf{not} multiplied by the Model-Size Multiplier --- they are added directly to the total. A team may receive 0, 15, or 30 bonus points per round.
...
\bottomrule
\end{tabular}
\caption{Performance Bonus points.}
\end{table}
...
\text{Total} &= (70 \times 1.05) + 12.75 + 30 - 0 = 73.5 + 12.75 + 30 = \mathbf{116.25}
\end{align*}

\textbf{Scenario 2:} Team uses the \texttt{large} model. The average mAP is 0.92. The robot recognizes but does not contact the Rocket once ($+15$), and incorrectly avoids two Tires by rotating counter-clockwise ($+5 \times 2$), correctly avoids one Tin Can by rotating counter-clockwise ($+15$). No avoid objects were physically touched. Two rescues were needed.

\begin{align*}
\text{Field Performance} &= 15 + 5 + 5 + 15 = 40 \\
\text{Model-Size Multiplier} &= 0.95 \\
\text{mAP Bonus} &= 0.92 \times 15 = 13.8 \\
\text{Performance Bonus} &= 0_{\text{(Rocket not touched)}} + 15_{\text{(Clean Avoid)}} = 15 \\
\text{Penalties} &= -15 \quad \text{(second rescue)} \\
\text{Total} &= (40 \times 0.95) + 13.8 + 15 - 15 = 38 + 13.8 + 15 - 15 = \mathbf{51.8}
\end{align*}

\textbf{Scenario 3:} Layout NP\_6 (2 Rockets + 2 Tires + 2 Tin Cans). Team uses the \texttt{nano} model. The average mAP is 0.70. The robot correctly contacts Rocket 1 ($+20$), correctly avoids Tire 1 by rotating clockwise ($+15$), then re-approaches Tire 1 and correctly avoids it again ($+15$), touches Tin Can 1 (false contact, $-10$), correctly contacts Rocket 2 ($+20$), correctly avoids Tin Can 2 by rotating counter-clockwise ($+15$), and never finds Tire 2 ($0$). No rescues were needed.

\begin{align*}
\text{Field Performance} &= 20 + 15 + 15 + (-10) + 20 + 15 + 0 = 75 \\
\text{Model-Size Multiplier} &= 1.05 \\
\text{mAP Bonus} &= 0.70 \times 15 = 10.5 \\
\text{Performance Bonus} &= 15_{\text{(both Rockets touched)}} + 0_{\text{(touched Tin Can 1)}} = 15 \\
\text{Penalties} &= 0 \\
\text{Total} &= (75 \times 1.05) + 10.5 + 15 - 0 = 78.75 + 10.5 + 15 = \mathbf{104.25}
\end{align*}

\textit{Note: Tire 1 was scored twice ($+15$ each time) --- objects may be engaged multiple times. The false contact on Tin Can 1 cost the team both $-10$ points and not having the $+15$ Clean Avoid bonus.}
...
\begin{enumerate}
    \item \textbf{False contact ($-10$ per incident)} --- The robot physically touches an avoid object (Tire or Tin Can). This is scored within Field Performance each time it occurs. Since objects may be engaged multiple times, a single object can incur multiple false contacts.
...
\addlinespace
Robot drives over an avoid object & Any physical contact with an avoid object counts as a false contact ($-10$), even without visible displacement. For a Rocket, contact without the full approach-nudge-reverse sequence does not score. \\
\addlinespace
...
\begin{enumerate}
    \item \textbf{Fewer total penalties.} Count the total number of penalty-incurring events: false contacts (each $-10$ within Field Performance) plus rescue penalties (each $-15$ after the first rescue). The team with fewer penalty events wins the tiebreaker.
...
\hline
\textbf{Position} & \textbf{Object Class} & \   textbf{Action Observed} & \textbf{Correct?} & \textbf{Points} \\
\hline
...
\hline
\textbf{D --- Performance Bonus} (Perfect Contact + Clean Avoid) & $+$ \rule{2cm}{0.4pt} \\
\hline
\textbf{E --- Penalties} (rescue penalties) & $-$ \rule{2cm}{0.4pt} \\
\hline
\textbf{TOTAL = (A $\times$ B) + C + D $-$ E} & \rule{3cm}{0.4pt} \\
\hline
\end{tabular}
\end{table}
