...
Object positions are physically fixed. Between heats, the referee assigns classes to positions using one of these templates. Both halves use the same template simultaneously.
...
\begin{itemize}
    ...
    \item The camera sees all 3 objects from the starting position (the partition does not block vision).
...
\item \textbf{Class rotation.} Object positions are physically fixed. Between heats, the referee changes the rotation template, which determines which class is assigned to each position. All teams in the same heat face the same assignment.
...
\item \textbf{Partition rules.} The partition is a physical barrier separating the two halves. Colliding with the partition or perimeter walls is \textbf{not a penalty}. Each robot must remain in its designated half. If a robot navigates through a gap into the other half, this is not a penalty but wastes time; the referee may note interference if it affects the other team.
...
\begin{enumerate}
    ...
    A GUI application (\texttt{gui\_train\_model.py}) lets you:
    \begin{itemize}
        ...
        \item Export the trained model to ONNX format (FP16), which generates the cryptographic receipt.
    \end{itemize}
...
\begin{enumerate}[label=\textbf{Q\arabic*:}, leftmargin=2.5em]
    ...
    \item \textbf{Do both teams in a heat face the same field?} \\
...
\midrule
Heat & A round in which multiple teams compete under the same field configuration \\
...
\begin{enumerate}
    ...
    \item Select rotation template for the heat (A, B, or C).
...
\begin{enumerate}
    ...
    \item Fill scorecard headers (team name, date, heat \#, half assignment).
...
\begin{enumerate}
    ...
    \item Verify all positions are correct before the next heat.
\end{enumerate}
...
\hline
\textbf{Layout} & P\_3 (Mirrored) & \textbf{Template} & A / B / C & \textbf{Round \#} & \\
\hline
