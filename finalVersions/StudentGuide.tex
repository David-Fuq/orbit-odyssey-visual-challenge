...
The crash site is enclosed by low barrier walls. The rover deploys from a \textit{Low Rubble Zone}: a safe corner of the crash site with minimal debris.
...
The challenge follows a four-stage pipeline. Your team is directly responsible for \textbf{Stage 1 and Stage 2}: training the model. The other stages use scripts provided by the organizers that \textbf{cannot be modified}.

\subsection*{The pipeline}

\begin{enumerate}
    \item \textbf{Data gathering}\\
    Each team must take photos of the objects for training. A total of 200 photos per object is recommended. This is a fundamental part, as it will be the only data your model will be given. 

    \item \textbf{Model training}  \\
    A GUI application that lets you:
    \begin{itemize}
        \item Select and merge datasets for up to 3 object classes.
        \item Choose a model variant (nano, small, medium, large, xlarge).
    ...
    Runs your trained model on a live webcam feed and sends detection results to the XRP robot over a serial (UART) connection. This script is identical for all teams. No team may modify this script. 

    \item \textbf{Robot behavior} \\
    The XRP robot receives detections and executes a state machine: search for objects, center on them, then contact or avoid based on the detected class. This script runs on MicroPython and is identical for all teams. No team may modify this script. 
\end{enumerate}

\begin{framed}
\textbf{Key takeaway:} The only difference between teams is the \textbf{dataset and the trained model}. A model that accurately detects and classifies objects will produce a rover that contacts the right objects and avoids the right debris. A model that confuses classes will cause the rover to turn the wrong way or touch hazards.
\end{framed}
...
For more precise details and explanations, see the \textit{Games Rules and Points System} document
...
Take images of all 3 classes (Rocket, Tire, Tin Can). Be sure to take as many as you need, in as many angles as you seem necessary.
...
For a step-by-step explanation, see the following tutorial in youtube \href{http}{YouTube tutorial will be here once we define everything related to the GUI}
...
\begin{framed}
\textbf{Tip:} You can train multiple models with different variants and compare their mAP scores. The only model that really matters is the one you use during the competition. Trial and error is a fundamental part of learning!
\end{framed}
...
\begin{enumerate}
    ...
    \item \textbf{Briefing.} The referee announces the start corner. All teams in the same heat use the same start position.
\end{enumerate}
