...
This layout uses \textbf{6 objects}: 2 Rockets (contact) + 2 Tires (avoid) + 2 Tin Cans (avoid). Each object has an approximate radius of 6 cm. The objects are split evenly across both halves of the field (\textbf{3 bottom, 3 top}) and arranged in a \textbf{high-low-high zigzag} pattern: side objects sit closer to the partition, center objects closer to the outer wall. This prevents the robot from getting boxed in by a horizontal line of objects.
...
\bottomrule
\end{tabular}
\caption{Object positions --- Layout P\_6 (zigzag).}
\end{table}
...
The layout is symmetric: each corner has one object at $\sim$43 cm, one at $\sim$89 cm, two at $\sim$112--155 cm, and two far objects at $\sim$195 and $\sim$210 cm. No corner has an advantage.

\subsection*{Navigation notes}

The camera sees all 6 objects from any corner (the low partition does not block vision). The partition only affects the driving path:

\begin{itemize}
    \item \textbf{From bottom corners:} P1, P2, P3 are directly reachable. P4, P5, P6 require going around the partition via the left gap ($X < 63.5$ cm) or right gap ($X > 190.5$ cm).
    \item \textbf{From top corners:} P4, P5, P6 are directly reachable. P1, P2, P3 require going around.
...
\bottomrule
\end{tabular}
\caption{Balanced templates --- one contact target per half.}
\end{table}
