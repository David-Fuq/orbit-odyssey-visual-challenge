...
This section applies to the \textbf{P\_3 layout} (center-start fallback, partition present, 3 objects in the bottom half). The rules below are exhaustive. Any situation not explicitly permitted is \textbf{prohibited}. If an ambiguity arises during competition, the head referee's ruling is final and binding.
...
The robot is placed \textbf{off-center in the bottom half} of the field at coordinates $(127.00,\; 22.86)$ cm $= (50,\; 9)$ in. This position is below and centered relative to the three objects, which are all in the bottom half. The robot must be placed at this position by the referee.
...
Objects are placed by the referee according to the active placement template. All three objects are located in the \textbf{bottom half} of the field (below the partition). The robot does not need to cross the partition to reach any object. Teams may \textbf{not} move, rotate, reposition, or in any way interfere with objects on the field.
...
\textbf{Note:} Even though the robot starts at an off-center position, rescues always place the robot in a corner zone --- not back at the starting position.

\subsection*{R11 --- Partition Rules}
The central partition is present in this layout:
\begin{itemize}
    \item All three objects are in the \textbf{bottom half}. The robot should not need to cross the partition during normal play.
    \item The partition does \textbf{not} block the camera's view (it is low PVC pipe).
    \item If the robot inadvertently drives into the top half via a gap, colliding with the partition is \textbf{not} a penalty.
\end{itemize}
...
\midrule
P\_3 & 1R + 1T + 1C & $20 + 15 + 15 + 15 + 15$ & 80 \\
\bottomrule
\end{tabular}
\caption{Maximum Field Performance score for P\_3.}
\end{table}
...
\textbf{Scenario:} Layout P\_3 (1 Rocket + 1 Tire + 1 Tin Can, all in bottom half). Team uses YOLOv11m (Medium tier). Average mAP = 0.78. The robot correctly contacts the Rocket ($+20$), correctly avoids both the Tire ($+15$) and Tin Can ($+15$). No avoid objects were physically touched. Two rescues needed.

\begin{align*}
\text{Field Performance} &= 20 + 15 + 15 + 15_{\text{(Perfect Contact)}} + 15_{\text{(Clean Avoid)}} = 80 \\
\text{Model-Size Multiplier} &= 1.00 \\
\text{mAP Bonus} &= \lfloor 0.78 \times 15 \rfloor = 11 \\
\text{Penalties} &= 15 \quad \text{(second rescue costs $-15$)} \\
\text{Total} &= (80 \times 1.00) + 11 - 15 = 80 + 11 - 15 = \mathbf{76}
\end{align*}
...
\addlinespace
Robot collides with the partition or a perimeter wall & \textbf{Not a penalty.} The robot's wall avoidance handles this automatically. \\
\addlinespace
Robot crosses into the top half via a gap & \textbf{Not a penalty.} However, there are no objects in the top half, so the robot is wasting time. The team may request a rescue. \\
\addlinespace
...
Scorecard for P\_3 layout (3 objects, off-center start, partition present).
...
\hline
\textbf{Layout} & P\_3 & \textbf{Template} & \rule{3cm}{0.4pt} \\
\hline
\end{tabular}
\end{table}
