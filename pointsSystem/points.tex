...
The robot must be placed \textbf{fully inside} the designated corner zone (30.48 $\times$ 30.48 cm). The corner is selected by the referee before each heat and is the same for all teams in that heat. No part of the robot may extend beyond the corner zone boundary at the moment of the start signal.
...
Objects are placed by the referee according to the active placement template. The positions are fixed and identical for all teams in a heat. Teams may \textbf{not} move, rotate, reposition, or in any way interfere with objects on the field. If a team member touches an object (other than through the robot during the run), the run is immediately terminated.

\subsection*{R9 --- Class Rotation}
Object positions (P1, P2, \ldots) are physically fixed for the entire event. Between heats, the referee changes which object class (Rocket, Tire, Tin Can) is assigned to which position by selecting a different rotation template. All teams in the same heat face the \textbf{same template}.
...
\subsection*{R11 --- Partition Rules (P\_5 and P\_6 layouts only)}
When the central partition is present:
\begin{itemize}
    \item The partition is a physical barrier that the robot cannot drive through. It must navigate around via the 25-inch gaps on either side.
    \item The partition does \textbf{not} block the camera's view (it is low PVC pipe).
    \item Colliding with the partition is \textbf{not} a penalty. The robot's wall-avoidance logic handles it automatically.
\end{itemize}

\subsection*{R12 --- Field Access}
...
\textbf{Definitions:}
\begin{itemize}
    \item \textbf{Nudge:} The robot physically pushes the object, displacing it $\geq$3 cm from its original position. Measured by the referee at the moment of contact.
...
\midrule
Perfect Contact & All Rockets on the field have been correctly contacted ($+20$ each) & $+15$ \\
...
\subsubsection*{Maximum Field Performance by layout}
...
Where $\text{avg\_mAP}$ is the arithmetic mean of the per-class mAP\textsubscript{50} values across all 3 classes (Rocket, Tire, Tin Can), as reported in the cryptographic receipt. For example, if the per-class mAP values are 0.92, 0.87, and 0.81, then $\text{avg\_mAP} = 0.867$ and $\text{mAP Bonus} = \lfloor 0.867 \times 15 \rfloor = 13$.
...
There is no out-of-bounds penalty --- the PVC pipe walls physically contain the robot. False contacts ($-15$) are counted within the Field Performance score, not as a separate penalty.

\subsection*{Worked Example}

\textbf{Scenario:} Layout NP\_5 (1 Rocket + 2 Tires + 2 Tin Cans). Team uses YOLOv11s (Small tier). Average mAP = 0.85. The robot correctly contacts the Rocket ($+20$), correctly avoids both Tires ($+15 \times 2$), correctly avoids one Tin Can ($+15$), and turns the wrong direction on the second Tin Can ($+5$). No avoid objects were physically touched. One rescue was needed.

\begin{align*}
\text{Field Performance} &= 20 + 15 + 15 + 15 + 5 + 15_{\text{(Perfect Contact)}} + 15_{\text{(Clean Avoid)}} = 100 \\
\text{Model-Size Multiplier} &= 1.05 \\
\text{mAP Bonus} &= \lfloor 0.85 \times 15 \rfloor = 12 \\
\text{Penalties} &= 0 \quad \text{(first rescue is free)} \\
\text{Total} &= (100 \times 1.05) + 12 - 0 = 105 + 12 = \mathbf{117}
\end{align*}
...
\begin{enumerate}
    \item \textbf{False contact ($-15$ per incident)} --- The robot physically nudges an avoid object (Tire or Tin Can) $\geq$3 cm. This is scored within Field Performance on a per-object basis. A single object can incur at most one false contact.
...
\addlinespace
Robot collides with the partition or a perimeter wall & \textbf{Not a penalty.} The robot's rangefinder-based wall avoidance handles this automatically. Collisions with walls are expected behavior. \\
\addlinespace
...
\addlinespace
Serial connection drops during the run & \textbf{Not a penalty.} The robot reverts to its search behavior. The team may request a rescue if the robot becomes stuck. \\
\addlinespace
...
\addlinespace
Robot drives over an object without displacing it & No interaction is recorded. The robot must visibly displace ($\geq$3 cm) or visibly approach and turn to score. \\
\addlinespace
Avoid object displaced less than 3 cm by incidental robot contact & \textbf{Not} a false contact. The 3 cm threshold must be met for any contact (positive or negative) to be scored. \\
\addlinespace
...
The cryptographic receipt system ensures that model metrics (mAP, model variant) are tamper-proof and were generated at training time, not fabricated after the fact.
...
\begin{enumerate}
    ...
    \item The judge reads the YOLOv11 variant from the receipt and determines the \textbf{model-size tier} (Small / Medium / Large).
