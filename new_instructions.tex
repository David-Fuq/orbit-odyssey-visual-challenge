...
The Orbit Odyssey Computer Vision Challenge was originally conceived and developed by Rushil Sambangi as an end-to-end machine learning pipeline challenge. The goal is to give students firsthand, hands-on experience with the full lifecycle of an ML model: data collection, model training, real-world deployment, and autonomous robot control — all within the span of a single activity.

The original pipeline was built around four key technologies: \textbf{Open Images} (Google's open-source labeled image dataset), \textbf{YOLOv11} (a state-of-the-art real-time object detection model), the \textbf{Intel AIxBoard} (a compact edge AI computer that runs the model during the game), and the \textbf{XRP robot} (a programmable differential-drive robot that acts on the detections).

\subsection{Adaptation for Orbit Odyssey}

This adaptation reimagines the challenge within a space exploration narrative, replacing the original object classes with thematically coherent alternatives and refining the competition rules and scoring system.

\subsubsection*{Mission Narrative}

A robotic scout has been dispatched to a lunar surface following the crash of an unmanned cargo spacecraft. The debris field is scattered with remnants of the ship's landing gear (\textbf{tires}) and canisters of preserved space food (\textbf{tin cans}). Somewhere in the wreckage, an intact \textbf{rocket} remains — the only means of signaling for rescue.

The mission: navigate through the debris, make contact with the rocket, and avoid disturbing the wreckage. The robot must complete this mission autonomously, guided only by its camera and its trained detection model.
...
Runs on the XRP microcontroller itself (written in MicroPython, uploaded via Thonny). Receives the JSON detection payloads from \texttt{run\_model.py} and implements a four-state behavior loop:

\begin{enumerate}
    \item \textbf{Wait} — motors off; robot holds still until the first detection packet arrives.
    \item \textbf{Search} — spins in place until a new object appears in the camera frame.
    \item \textbf{Center} — turns left or right until the detected object is within a pixel threshold of the frame center.
    \item \textbf{Contact or Avoid} — drives toward the object using the rangefinder for distance, then either nudges it (contact) or turns away (avoid).
\end{enumerate}

This script is fully provided to students and runs automatically when the XRP is powered on.
...
As a student, your responsibility is to \textbf{train the best possible detection model} using the provided datasets and graphical training tool. You do not write the robot control code — that is handled by the provided \texttt{main.py}. Your decisions during training directly affect your score.
...
\begin{enumerate}
    ...
    \item Configure your training parameters (see Section~\ref{sec:scoring_strategy} for guidance).
...
%===========================================================
\section{Alternative Game Mode: Debris Collection Sprint}
%===========================================================
...
This mode shifts the emphasis from model accuracy alone to the combination of model accuracy and robot response speed, and produces a more dramatic, race-like experience that is easy to judge and fun to watch.
...
The concept of ``collecting'' replaces ``avoiding'': instead of steering away from objects, the robot approaches each one and turns in the designated direction, simulating the act of clearing the debris from its path before continuing toward the rocket.

\subsection{Course Layout}

The course is a black line path designed by the moderator. Unlike the original game mode, \textbf{object placement is fully controlled by the moderator} — objects are placed at junctions along the path, not in random zones. This makes every team's run identical and directly comparable.
...
\begin{enumerate}
    ...
    \item \textbf{Collect} — robot turns in the designated direction (left for Tin can, right for Tire). A successful turn is counted as a valid collection.
    \item \textbf{Resume} — robot reacquires the line and continues.
    \item \textbf{Finish} — upon detecting the Rocket, robot drives straight and contacts it. The timer stops on contact.
\end{enumerate}

\subsection{Timing and Adjustments}

The base score is the \textbf{elapsed time from run start to rocket contact}. The following adjustments are applied:
...
The team with the \textbf{lowest adjusted time} wins. If a team fails to reach the rocket within the time limit, they are ranked below all teams that completed the course, ordered by the number of objects correctly collected.

\subsection{Tiebreakers}

\begin{enumerate}
    \item More objects correctly collected
    \item Higher average mAP (model accuracy)
    \item Smaller model family
\end{enumerate}
...
\begin{itemize}
    \item The line path should be designed so that every junction has a clear approach angle — the camera should have a direct view of the object before the robot reaches the turn point.
    ...
    \item The course difficulty can be tuned by adjusting the number of junctions, the tightness of turns, or the length of straight segments between objects.
    \item For fairness, all teams run the \textbf{exact same course} in the same match session. The moderator resets object positions between runs.
\end{itemize}
