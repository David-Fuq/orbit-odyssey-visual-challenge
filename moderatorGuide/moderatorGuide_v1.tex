...
This guide provides step-by-step instructions for referees running the Orbit Odyssey Computer Vision Challenge. In this challenge, student teams train a YOLOv11 object detection model and deploy it on an Intel AIxBoard, which streams detections to an XRP robot over serial. The robot must autonomously contact or avoid 3D-printed objects (Rockets, Tires, and Tin Cans) during a 120-second game run.
...
This guide is self-contained --- you do not need to read the full rules document to referee a match. However, for edge cases or disputes, refer to the \textit{Rules and Points System} document.
...
Layout & The physical arrangement (NP\_5, NP\_6, P\_5, P\_6, NP\_4, or P\_3) \\
...
Low Rubble Zone & A 30.48 $\times$ 30.48 cm (12 $\times$ 12 in) corner zone; the robot's starting area \\
...
\begin{enumerate}[label=$\square$]
    ...
    \item \textbf{Partition} (if using P\_5, P\_6, or P\_3) --- 127 cm PVC barrier, same height as perimeter walls.
    \item \textbf{3D-printed objects} --- Rockets, Tires, and Tin Cans. Quantities depend on layout:
    \begin{itemize}
        \item 3-object layouts (NP\_4, P\_3): 1 Rocket + 1 Tire + 1 Tin Can.
        \item 5-object layouts (NP\_5, P\_5): 1 Rocket + 2 Tires + 2 Tin Cans.
        \item 6-object layouts (NP\_6, P\_6): 2 Rockets + 2 Tires + 2 Tin Cans.
    \end{itemize}
    \item \textbf{Printed scorecards} --- at least one per team per run. Use the standalone scorecard template (\texttt{scorecard.tex}).
    \item \textbf{Timer} --- capable of a 120-second countdown with an audible buzzer.
    \item \textbf{Measuring tape or ruler} --- for verifying object placement and the 3 cm displacement threshold.
    \item \textbf{Layout reference sheet} --- printed copy of the active field definition document (from \texttt{fieldDefinition/}) with object positions in cm and inches.
    \item \textbf{Rotation template list} --- printed copy of the available templates for the chosen layout.
    \item \textbf{SHA-256 hash tool} --- a laptop or phone with a hash calculator for verifying the ONNX file hash on the AIxBoard.
...
\begin{enumerate}
    ...
    \item \textbf{Select rotation template.} Choose a template from the layout's rotation table. Use a \textbf{different template for each heat} so that teams cannot memorize object-class assignments.
...
Perform this procedure \textbf{before each team's run}. The mAP Bonus and Model-Size Multiplier must be recorded on the scorecard before the timer starts.

\begin{enumerate}
    \item \textbf{Request the receipt.} Ask the team to present their cryptographic receipt. The receipt was generated automatically by the training pipeline at ONNX export time.
    ...
    \begin{equation*}
    \text{mAP Bonus} = \text{avg}(\text{Rocket}_{\text{mAP}},\;\text{Tire}_{\text{mAP}},\;\text{Can}_{\text{mAP}}) \times 15 \qquad (\text{max } +15)
    \end{equation*}
...
\begin{enumerate}
    ...
    \item \textbf{Announce start position.} Tell the team which corner (or center position) to use. For corner-start layouts, announce the specific corner (e.g., ``Bottom-Left corner''). For center-start layouts, indicate the center placement coordinates.
...
\begin{itemize}
    ...
    \item An action that is still in progress when the buzzer sounds does \textbf{not count}. ``Completed'' means: for contact, the object has been displaced $\geq$3 cm; for avoidance, the robot has completed its turn.
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Acknowledge.} Say ``Rescue acknowledged'' out loud.

    \item \textbf{Team member enters field.} Only after your acknowledgment may one team member enter the field, pick up the robot, and place it in \textbf{any of the Low Rubble Zone corners} (for corner-start layouts) or any corner (for center-start layouts).
    ...
    \begin{itemize}
        \item \textbf{First rescue:} free (no point penalty, only time cost).
    ...
    \begin{itemize}
        ...
        \item A $-15$ point penalty is applied.
...
\addlinespace
Action in progress when buzzer sounds & Counts \textbf{only if completed} before the buzzer. Contact: object displaced $\geq$3 cm. Avoid: robot has completed its turn. \\
\addlinespace
...
\addlinespace
Rocket contact without full sequence & Contact without approach-touch-reverse does \textbf{not} score. The full sequence must be observed. \\
\addlinespace
